...
document.getElementById("turn").innerText = "It is player " + currentTurn + "'s turn";//or we can use string template `It is player ${currentText} 's turn`
...
function checkForWin() {
    ...
    //check for columns
    for (var j = 0; j < 3; j += 1) {
        if (squares[j].innerText && squares[j].innerText === squares[j + 3].innerText && squares[j + 3].innerText === squares[j + 2].innerText) {
            declareWinner();
            return true;
        }
    }
